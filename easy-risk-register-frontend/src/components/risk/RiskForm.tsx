...
export const RiskForm = forwardRef<RiskFormHandle, RiskFormProps>(({
  ...
  // Only reset form when the riskId or mode actually changes, 
  // or if explicitly requested via a change in initial data structure
  const lastRiskIdRef = useRef(riskId)
  useEffect(() => {
    if (lastRiskIdRef.current !== riskId) {
      lastRiskIdRef.current = riskId
      reset({
  ...
  const onFormSubmit = useCallback(async (values: RiskFormValues) => {
    console.debug('[RiskForm] Form submitted with values:', {
    ...
    >
      <div className="flex flex-1 flex-col gap-4 pb-1">
        <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)] xl:grid-cols-[minmax(0,1fr)]">
          <div className="rounded-2xl border border-border-subtle bg-surface-primary p-8 shadow-sm transition-all duration-300 hover:shadow-md">
            <div className="flex items-center gap-3 mb-8">
              <div className="p-2.5 rounded-2xl bg-brand-primary text-white shadow-lg shadow-brand-primary/20">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"></path></svg>
              </div>
              <div>
                <h3 className="text-lg font-bold text-text-high leading-tight">Basics & Scope</h3>
                <p className="text-xs text-text-low">Identity and classify this risk scenario</p>
              </div>
            </div>
          ...
          {/* Scoring Section */}
          <div className={cn("mt-6 rounded-3xl border border-border-faint bg-gradient-to-br from-white to-surface-secondary/30 p-8 shadow-sm transition-all duration-500",
            severity === 'low' ? 'shadow-[0_0_30px_-10px_rgba(34,197,94,0.15)] bg-risk-low/5' :
              severity === 'medium' ? 'shadow-[0_0_30px_-10px_rgba(249,115,22,0.15)] bg-risk-medium/5' :
                'shadow-[0_0_30px_-10px_rgba(239,68,68,0.15)] bg-risk-high/5')}>

            <div className="flex items-center justify-between mb-8">
              <div className="flex items-center gap-3">
                <div className={cn("p-2.5 rounded-2xl text-white shadow-lg",
                  severity === 'low' ? 'bg-risk-low shadow-risk-low/20' :
                    severity === 'medium' ? 'bg-risk-medium shadow-risk-medium/20' :
                      'bg-risk-high shadow-risk-high/20')}>
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"></path><line x1="12" y1="9" x2="12" y2="13"></line><line x1="12" y1="17" x2="12.01" y2="17"></line></svg>
                </div>
                <div>
                  <h3 className="text-lg font-bold text-text-high leading-tight">Quantification</h3>
                  <p className="text-xs text-text-low">Probability × Impact = Risk Exposure</p>
                </div>
              </div>
              <Badge tone={severityMeta.pillTone.includes('success') ? 'success' : severityMeta.pillTone.includes('warning') ? 'warning' : 'danger'} className="px-4 py-1.5 font-bold uppercase tracking-widest animate-pulse-slow">
            ...
            <div className="grid gap-12 lg:grid-cols-[1fr_20rem]">
              <div className="flex flex-col gap-8">
                <div className="space-y-5">
                  <div className="flex items-center justify-between">
                    <span className="flex items-center gap-2.5 text-sm font-bold text-text-mid">
                      Likelihood
                      {showTooltips && (
                        <Tooltip
                          content="Probability of occurrence over 12 months."
                          ariaLabel="Likelihood help"
                        />
                      )}
                    </span>
                    <span className="text-3xl font-black text-brand-primary tabular-nums">
                      {probability} <span className="text-[10px] text-text-low font-bold">/ 5</span>
                    </span>
                  </div>
                  <Controller
                    name="probability"
                    ...
                    render={({ field }) => (
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          field.onChange(val)
                          console.debug('[RiskForm] Probability sync:', val)
                        }}
                        className="rr-slider w-full"
                        aria-label="Likelihood (1-5)"
                      />
                    )}
                  />
                  <div className="flex justify-between text-[10px] font-black uppercase tracking-[0.2em] text-text-muted/60 px-1">
                    <span>Rare</span>
                    <span>Certain</span>
                  </div>
                </div>
                ...
                <div className="space-y-5">
                  <div className="flex items-center justify-between">
                    <span className="flex items-center gap-2.5 text-sm font-bold text-text-mid">
                      ...
                      {showTooltips && (
                        <Tooltip
                          content="Worst credible outcome if the risk occurs."
                          ariaLabel="Impact help"
                        />
                      )}
                    </span>
                    <span className="text-3xl font-black text-brand-primary tabular-nums">
                      {impact} <span className="text-[10px] text-text-low font-bold">/ 5</span>
                    </span>
                  </div>
                  <Controller
                    name="impact"
                    ...
                    render={({ field }) => (
                      <input
                        ...
                        onChange={(e) => {
                          const val = Number(e.target.value)
                          field.onChange(val)
                          console.debug('[RiskForm] Impact sync:', val)
                        }}
                        className="rr-slider w-full"
              ...
              <div className="relative flex flex-col items-center justify-center p-8 rounded-3xl bg-white shadow-inner-lg border border-border-faint overflow-hidden">
                <div className="absolute inset-0 bg-gradient-to-b from-brand-primary/[0.02] to-transparent pointer-events-none"></div>
                <p className="text-[10px] font-black uppercase tracking-[0.4em] text-text-low/60 mb-3">Live Risk Score</p>
                <div className="flex items-baseline">
                  <span className={cn("text-8xl font-black transition-all duration-300 transform scale-100",
                    severity === 'low' ? 'text-risk-low drop-shadow-[0_4px_12px_rgba(34,197,94,0.3)]' :
                      severity === 'medium' ? 'text-risk-medium drop-shadow-[0_4px_12px_rgba(249,115,22,0.3)]' :
                        'text-risk-high drop-shadow-[0_4px_12px_rgba(239,68,68,0.3)]')}>
                    {riskScore}
                  </span>
                </div>
                <p className="mt-4 text-xs font-bold text-text-low/80 animate-in slide-in-from-bottom-2 duration-500">
                  Recommendation: <span className="text-text-high">{severityMeta.nudge.replace('Next: ', '')}</span>
                </p>
              </div>
            </div>
          </div>
      ...
      <details open className="group space-y-4 rounded-2xl border border-border-subtle bg-surface-primary p-2 shadow-sm transition-all hover:shadow-md open:p-6">
        <summary className="flex cursor-pointer list-none items-center justify-between rounded-xl bg-surface-secondary/30 px-4 py-3 text-sm font-semibold text-text-high transition-colors hover:bg-surface-secondary/50 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-brand-primary/20 group-open:bg-transparent">
          <span className="flex items-center gap-3">
            <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-brand-primary/10 text-brand-primary">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M12 2v20M2 12h20"></path></svg>
            </div>
            Plan (optional)
            <span className="ml-2 rounded-full bg-surface-secondary px-2 py-0.5 text-[10px] font-medium text-text-low">
              {formValues.mitigationPlan?.trim() || mitigationStepsArray.fields.length > 0 ? 'Active' : 'Empty'}
            </span>
          </span>
      ...
      <details className="group space-y-4 rounded-2xl border border-border-subtle bg-surface-primary p-2 shadow-sm transition-all hover:shadow-md open:p-6">
        <summary className="flex cursor-pointer list-none items-center justify-between rounded-xl bg-surface-secondary/30 px-4 py-3 text-sm font-semibold text-text-high transition-colors hover:bg-surface-secondary/50 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-brand-primary/20 group-open:bg-transparent">
            <span className="flex items-center gap-3">
              <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-brand-primary/10 text-brand-primary">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M13 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V9z"></path><polyline points="13 2 13 9 20 9"></polyline></svg>
              </div>
              Evidence & Extras (optional)
              <span className="ml-2 rounded-full bg-surface-secondary px-2 py-0.5 text-[10px] font-medium text-text-low">
                {evidenceArray.fields.length > 0 || playbookEnabled ? 'Active' : 'Empty'}
              </span>
            </span>
          <ChevronIcon className="h-5 w-5 text-text-muted transition-transform duration-200 group-open:rotate-180" />
        </summary>
