...
describe('ROSICalculator', () => {
  const mockOnROSIComputed = vi.fn()
  ...
  it('renders correctly', () => {
    ...
    expect(
      screen.getByText('Return on Security Investment (ROSI) Calculator')
    ).toBeInTheDocument()
    expect(
      screen.getByText('Calculate the return on investment for security controls')
    ).toBeInTheDocument()
    expect(
      screen.getByText('Annual Loss Expected (ALE) without control ($)')
    ).toBeInTheDocument()
    expect(
      screen.getByText('Annual Loss Expected (ALE) with control ($)')
    ).toBeInTheDocument()
  ...
  it('validates control cost', () => {
    ...
    expect(alertSpy).toHaveBeenCalledWith('Control cost must be greater than zero')
  })
})
