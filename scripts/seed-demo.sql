...
select public.create_workspace(:'workspace_name') as workspace_id;
\gset

insert into public.workspace_members (workspace_id, user_id, role)
values
  (:'workspace_id'::uuid, :'admin_id'::uuid, 'admin'),
  (:'workspace_id'::uuid, :'member_id'::uuid, 'member')
...
values (:'workspace_id'::uuid, true, 30, 7, 20)
...
values
  (:'workspace_id'::uuid, :'owner_id'::uuid, true, true, true),
  (:'workspace_id'::uuid, :'admin_id'::uuid, true, false, true),
  (:'workspace_id'::uuid, :'member_id'::uuid, true, false, false)
...
select
  coalesce(
    ...
    (select rc.id
     ...
     where rc.workspace_id = :'workspace_id'::uuid
...
select public.refresh_risk_checklist_status(:'workspace_id'::uuid, :'checklist_id'::uuid);
...
select
  coalesce(
    ...
    (select rp.id
     ...
     where rp.workspace_id = :'workspace_id'::uuid
...
values
  (:'workspace_id'::uuid, :'risk_phish_id'::uuid, 'risk_created', jsonb_build_object('demo', true)),
  (:'workspace_id'::uuid, :'risk_privacy_id'::uuid, 'checklist_attached', jsonb_build_object('demo', true, 'template_id', 'checklist_privacy_incident_ndb_v1')),
  (:'workspace_id'::uuid, :'risk_privacy_id'::uuid, 'playbook_attached', jsonb_build_object('demo', true, 'template_id', 'playbook_privacy_incident_ndb_v1')),
  (:'workspace_id'::uuid, null, 'maturity_assessed', jsonb_build_object('demo', true, 'assessment_id', :'assessment_id'::uuid));
